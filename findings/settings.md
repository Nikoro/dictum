...
**Root cause:** `llmDownloadedModelId` is set during onboarding when the user downloads an LLM model, but is never cleared when the model is deleted via `DownloadedLLMModelStore`. This is intentional — only the STT model gate (`whisperDownloadedModelIds`) is authoritative for `isSetupComplete`. The LLM download step in onboarding is optional.

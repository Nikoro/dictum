...
3. Replace WhisperKit with an MLX-based STT (`mlx-audio-swift`, `FluidAudio`). Rejected for now: large refactor across `TranscriptionEngine`, `WhisperModelStore`, `STTModelSection`, Hugging Face model search, cache layout, and setup flow — not worth the risk for a single feature when WhisperKit works well for Polish.
